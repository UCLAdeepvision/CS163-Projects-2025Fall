...
It is also observed that Approach 3 did not work as well as we expected (with a 0.124% mIoU score improvement, which is 0.52% lower than Approach 1 and 0.63% lower than Approach 2). We identify two potential reasons. First, soft IoU loss directly optimizes the intersection-over-union ratio through smooth gradients, providing stable global-shape supervision that complements SSIM's boundary focus. Lovász-Softmax, while also a valid IoU surrogate, uses a fundamentally different gradient mechanism based on sorting prediction errors by magnitude—this may interact less favorably with SSIM's local window-based gradients. Second, the explicit class weighting (5× for fence/pole/sign) in the cross-entropy loss may conflict with Lovász's own error-magnitude weighting scheme, potentially causing gradient instability rather than balanced learning.
