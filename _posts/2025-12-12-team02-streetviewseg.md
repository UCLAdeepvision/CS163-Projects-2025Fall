...
For the SSIM-Lovasz-based approach in Approach 3, although the result is quite different from the other two approaches, but they share the same per-class IoU pattern, and the reason is also similar. It is similar to these two approaches that it performs better on those 4 really small classes and worse on the 2 relatively larger classes. Because in this approach both the SSIM loss term and the Lovasz loss term are boundary-aware terms, so adding Lovasz term and removing the IoU loss term is attempting to add more boundary-aware focus to the model. Therefore, the overall model focus is shifted toward the fine structures, causing some degradation on some relatively large homogeneous regions
...
It is also observed that Approach 3 did not work as well as we expected (with a 0.124% mIoU score, which is 0.52% lower than Approach 1 and 0.63% lower than Approach 2), it appears that the model performs the worst across all classes as well as the mIoU when we compare to the other two approaches, even though we replace the IoU loss with Lovasz loss which create more boundary-aware focus. We suspect that the primary reason is that both SSIM and Lovász loss aggressively target boundary/hard pixels, creating redundant gradients that destabilize learning, causing the model performance on all classes to degrade compared to the other two approaches — while soft IoU provided complementary global-shape supervision that balanced SSIM's boundary focus.
