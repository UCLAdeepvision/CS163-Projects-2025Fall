...
**Added:**
- `assets/images/new-pic.png`

**Deleted:**
- `assets/images/old-pic.png`
...
**自动移除机制**：
```
接受PR → 立即从本地列表移除 → 避免重复处理
跳过PR → 保留在列表中 → 可以稍后再审查
```

**刷新过滤机制**：
```bash
# 刷新时强制从GitHub重新获取，只获取开放的PR
curl "https://api.github.com/repos/OWNER/REPO/pulls?state=open&per_page=100"
                                                    ↑↑↑↑↑↑↑↑↑↑
                                                只返回open状态的PR
                                           (已关闭/已合并的自动过滤)
```
...
- ✅ 强制从GitHub获取最新数据
- ✅ 忽略本地缓存
- ✅ 自动过滤 `state=closed` 的PR
- ✅ 更新统计信息（总PR数）

**列表状态**：
- ✅ **接受**：自动push，从列表移除，GitHub上closed（刷新不出现）
- ⏭️ **跳过**：保留在列表，GitHub上open（继续显示）

**工作流示例**：
```
工作流程：
  - 启动工具，看到10个open PR
  - 接受PR #7 → 自动合并+发布评论+push → 从列表移除（列表剩9个）
  - 跳过PR #8 → 保留在列表（列表还是9个）
  - 接受PR #9 → 自动合并+发布评论+push → 从列表移除（列表剩8个）

刷新测试：
  - 点击"刷新" → 看到8个PR
    ✓ PR #7不出现（已自动push，GitHub上已closed）
    ✓ PR #8仍在列表（跳过的）
    ✓ PR #9不出现（已自动push，GitHub上已closed）
    ✓ 新提交的PR自动出现
```
